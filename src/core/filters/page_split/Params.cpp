...
Params::Params(const PageLayout& layout,
               const Dependencies& deps,
               const AutoManualMode splitLineMode,
               const QString& decisionReason)
    : m_layout(layout), m_deps(deps), m_splitLineMode(splitLineMode), m_decisionReason(decisionReason) {}
...
Params::Params(const QDomElement& el)
    : m_layout(el.namedItem("pages").toElement()),
      ...
      m_decisionReason(el.attribute("decisionReason")) {}
